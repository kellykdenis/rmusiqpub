...
export function BrowseNewReleases() {
  ...
  useEffect(() => {
    async function loadReleases() {
      try {
        ...
        const sortedAlbums = allNewReleases.sort(
          (a, b) => new Date(b.releaseDate).getTime() - new Date(a.releaseDate).getTime(),
        )

        // Display first page (20 items)
        const firstPageAlbums = sortedAlbums.slice(0, 20)
        setDisplayedAlbums(firstPageAlbums)

        // Set up pagination
        setPagination({
          currentPage: 1,
          totalPages: Math.ceil(sortedAlbums.length / 20),
          hasMore: sortedAlbums.length > 20,
  ...
  return (
    <div className="max-w-[1180px] mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-8">Discover Our Music</h1>
      <p className="text-zinc-600 mb-6">Showing releases arranged by date, newest first</p>
