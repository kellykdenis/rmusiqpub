...
// Replace the artistIdToSlugMap with a function that can generate slugs for new artists
...
const artistIdToSlugMap: Record<string, string> = {
  ...
  "4cZJRyLjg00Lof3gXteZVz": "new-artist", // New artist with a generated slug
}

// Function to generate a slug from an artist name
function generateSlug(name: string, id: string): string {
  // If we have a predefined slug, use it
  if (artistIdToSlugMap[id]) {
    return artistIdToSlugMap[id]
  }
...
export function ArtistsGrid() {
  ...
  // Update the useEffect hook to ensure it properly fetches and displays the new artist
  useEffect(() => {
        ...
        if (data && Array.isArray(data)) {
          ...
          data.forEach((spotifyArtist: SpotifyArtist) => {
            ...
            if (existingArtist) {
              ...
              const slug = generateSlug(spotifyArtist.name, spotifyArtist.id)
          ...
          const sortedArtists = updatedArtists.sort((a, b) => a.name.localeCompare(b.name))
